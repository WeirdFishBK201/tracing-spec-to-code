# CP-10 — Authority-channel wording rebuild
...
CP-09 produced 16 fresh, independently identifiable Codex runs. Fifteen matched
...
`EVALUATION_FAILED` because no Approved proposal and complete superseding
`5/5` rebuild exist.
...
Keep all 16 CP-09 active records unchanged. Add five fresh schema-v2 records:
...
single `supersedes_run_ids` entry, with `change_proposal: "CP-10"`. The five
...
Extend the deterministic validator with an exact CP-10 mapping. It must:

- Continue validating the 16 CP-09 archive reruns exactly as before.
- Treat CP-10 sources as active evidence, not CP-09 archive entries.
- Derive the Approved proposal link from the exact CP-10 replacement records;
  do not edit the already-recorded source to backfill `change_proposal_id`.
- Accept supersession of otherwise passing group members only when the complete
  five-entry CP-10 mapping exists, the group contains the recorded
  `new-rationale` source, CP-10 is Approved, and all replacements are later
  same-case passes.
...
new rationale, stop and require another Gate Δ change; do not rerun until a
...
- Evidence: add five files under `evaluation/evidence/`; do not edit or move
  existing CP-09 evidence or archive files.
- Plans: update the M05 plan and roadmap with the failed CP-09 observation,
  CP-10 gate, exact rebuild files, and actual verification.
...
1. CP-10 `prepare` accepts only the exact five run/source pairs.
2. Partial, extra, cross-case, unapproved, or non-passing rebuilds fail closed.
3. CP-09 archive validation remains unchanged.
...
excluded from effective counts only after the complete Approved CP-10 mapping
passes validation. All other CP-09 records remain effective and unchanged.

## Gate Δ

The user approved the authority-channel design direction and instructed
implementation of this written proposal on 2026-07-31.
